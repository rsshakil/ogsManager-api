...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (pathParameters === null) {
            ...
            const additionalConditionStr = ' userPaymentHistoryPaymentPattern = 6 ';
            ...
            SELECT COUNT(*) AS total_rows FROM (
                ...
                LEFT OUTER JOIN (
                    ...
                    FROM 
                    (
                    SELECT userPaymentHistoryIPAddress1 AS IP, userPaymentHistoryUserId FROM UserPaymentHistory WHERE userPaymentHistoryPaymentPattern = 6 AND userPaymentHistoryIPAddress1 IS NOT NULL
                    UNION
                    SELECT userPaymentHistoryIPAddress2 AS IP, userPaymentHistoryUserId FROM UserPaymentHistory WHERE userPaymentHistoryPaymentPattern = 6 AND userPaymentHistoryIPAddress2 IS NOT NULL
                    UNION
                    SELECT userPaymentHistoryIPAddress3 AS IP, userPaymentHistoryUserId FROM UserPaymentHistory WHERE userPaymentHistoryPaymentPattern = 6 AND userPaymentHistoryIPAddress3 IS NOT NULL
                    ) AS CountTable
                ...
                LEFT OUTER JOIN (
                    ...
                    FROM 
                    UserPaymentHistory
                    WHERE userPaymentHistoryPaymentPattern = 6 AND (userPaymentHistoryStatus = 1 OR userPaymentHistoryStatus = 4 OR userPaymentHistoryStatus = 12)
                    GROUP BY userPaymentHistoryUserId
            ...
            const sql_data = `
                SELECT 
                    ...
                    User.userEmail,
                    CASE WHEN userPaymentHistoryStatus = 1 THEN '成功' 
                    WHEN userPaymentHistoryStatus = 4 THEN '失敗' 
                    WHEN userPaymentHistoryStatus = 12 THEN '開始' 
